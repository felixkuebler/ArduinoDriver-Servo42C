...
public:
   ...
   */
  void Servo42CDriver::setZeroMode() {
    uint8_t cmdLen = 2;
    ...
    this->readResponse(this->uartId, resp, respLen);
  }
   ...
   * Return e0 , result (uint8_t), rCHK.
   */
  void Servo42CDriver::setZeroSpeed() {
    uint8_t cmdLen = 2;
   ...
   * Return e0 , result (uint8_t), rCHK.
   */
  void Servo42CDriver::moveToZero() {
    uint8_t cmdLen = 2;
   ...
   */
  void Servo42CDriver::moveSpeed(int16_t rpm) {
    // if rpm is set to zero use stop command
    if (rpm == 0) {
      uint8_t cmdLen = 1;
      uint8_t cmd[cmdLen] = {this->UartCommands::MoveSpeedStop};
      this->writeCommand(this->uartId, cmd, cmdLen);
  ...
  * @param rpm the speed of rotation, also sets the direction of rotation
  */
  void Servo42CDriver::moveAngleRelative(double angle, int16_t rpm) {
    ...
    uint32_t steps = (angle/360)*this->microStep*this->stepsPerRevolution;
    ...
    this->readResponse(this->uartId, respStart, respLen);

    // poll end status of rotation
    uint8_t respEnd[respLen] = {0x00};
    this->readResponse(this->uartId, respEnd, respLen);
  }
  ...
  */
  void Servo42CDriver::readAngleEncoder(double& angle) {
    // if rpm is set to zero no action needs to be taken
...
/*
    this->debug->print("Carry: ");
    this->debug->println(carry);

    this->debug->print("Value: ");
...
*/
  ...
  */
  void Servo42CDriver::readAngleShaft(double& angle) {
    // if rpm is set to zero no action needs to be taken
...
protected:
  ...
  void Servo42CDriver::readResponse(uint8_t id, uint8_t* resp, uint8_t respLen) {
